...
*/
class NFTOverlayManager extends UIModTemplate {
  constructor(app) {
    super(app);

    this.app = app;
    this.name = 'NFTOverlayManager';
  ...
  async onConfirmation(blk, tx, conf) {
    if (Number(conf) == 0) {
      ...
      if (txmsg.module == 'NFT') {
        console.log(
          `UI Component NFTOverlayManager sees a NFT-marked transaction in Block ${blk.id}!!!`
        );
        console.log(txmsg);
      }
    }
...
module.exports = NFTOverlayManager;
